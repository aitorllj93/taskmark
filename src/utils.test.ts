...
describe("markAsCompleted", () => {
	it("should mark a simple task as completed", () => {
		...
		expect(result).toMatch(
			/^- \[x\] Buy groceries #Tasks\/Quick ✅ \d{4}-\d{2}-\d{2}$/,
		);
	});
	...
	it("should handle task with other metadata", () => {
		...
		const result = markAsCompleted(task, "2025-01-20");
		expect(result).toBe(
			"- [x] Buy groceries #Tasks/Quick 🌡️ high ⏱️ 30m ✅ 2025-01-20",
		);
	});

	it("should handle task with dates", () => {
		const task =
			"- [ ] Task #Tasks/Quick ➕ 2025-01-10 ⏳ 2025-01-15 📅 2025-01-20";
	...
	it("should handle task with in_progress state", () => {
		...
		const result = markAsCompleted(task, "2025-01-20");
		expect(result).toBe(
			"- [x] Working on task #Tasks/Main_Mission ✅ 2025-01-20",
		);
	});
...
describe("setScheduledDate", () => {
	...
	it("should validate date format", () => {
		...
		expect(() => setScheduledDate(task, "invalid-date")).toThrow(
			"Invalid date format",
		);
		expect(() => setScheduledDate(task, "2025/01/20")).toThrow(
			"Invalid date format",
		);
		expect(() => setScheduledDate(task, "25-01-20")).toThrow(
			"Invalid date format",
		);
	});
	...
	it("should handle task with multiple dates", () => {
		const task =
			"- [ ] Task #Tasks/Quick ➕ 2025-01-10 🛫 2025-01-12 📅 2025-01-15";
...
describe("setTimeRange", () => {
	...
	it("should validate time format", () => {
		const task = "- [ ] Task #Tasks/Quick";
		expect(() => setTimeRange(task, "9:00", "10:30")).toThrow(
			"Invalid time format",
		);
	});
});
