...
describe('stringify - Tags', () => {
  ...
  it('should stringify task with scenario tags', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick #Scenarios/Work/Programming');
  });
  ...
  it('should stringify task with custom tags', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick #bug #priority #frontend');
  });
  ...
  it('should preserve tag order', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick #first #second #third');
  });
});
...
describe('stringify - Focuses', () => {
  ...
  it('should stringify multiple focuses', () => {
    ...
    expect(stringify(task)).toBe('- [ ] 🎯🔥🧠 Complex task #Tasks/Main_Mission');
  });
...
describe('stringify - Scenarios', () => {
  ...
  it('should stringify nested scenarios', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick #Scenarios/Work/Intense/Programming');
  });
  ...
  it('should stringify multiple scenarios', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick #Scenarios/Work #Scenarios/Home');
  });
});
...
describe('stringify - Recurrence', () => {
  it('should stringify recurrence rule', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick 🔁 every week on Monday');
  });
  ...
  it('should stringify complex recurrence rule', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick 🔁 every 2 weeks on Tuesday');
  });
...
describe('stringify - Dependencies', () => {
  ...
  it('should stringify multiple on completion dependencies', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick 🏁 start:abc123 🏁 notify:def456');
  });
  ...
  it('should stringify all dependency types together', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task #Tasks/Quick 🆔 task001 ⛔ task002 🏁 start:task003');
  });
});
...
describe('stringify - Edge Cases', () => {
  ...
  it('should handle content with special characters', () => {
    ...
    expect(stringify(task)).toBe('- [ ] Task with @mentions and #hashtags but valid #Tasks/Quick');
  });
...
describe('stringify - StringifyConfig', () => {
  it('should support custom typeTagMapping', () => {
    ...
    expect(stringify(task, config)).toBe('- [ ] Tarea #Tareas/Mision_Principal');
  });
  ...
  it('should support custom scenarioPrefix', () => {
    ...
    expect(stringify(task, config)).toBe('- [ ] Tarea #Tasks/Quick #Escenarios/Trabajo');
  });
  ...
  it('should round-trip with custom config (Spanish)', () => {
    const original = '- [ ] 🎯 Implementar autenticación #Tareas/Mision_Principal';
    ...
    expect(reparsed.content).toBe(parsed.content);
    expect(reparsed.type).toBe(parsed.type);
    expect(stringified).toBe('- [ ] 🎯 Implementar autenticación #Tareas/Mision_Principal');
  });

  it('should round-trip with custom scenarios config', () => {
    const original = '- [ ] Task #Tasks/Quick #Escenarios/Trabajo #Escenarios/Casa';
    const parseConfig: ParseConfig = {
      scenarioPrefix: 'Escenarios/',
      scenarioMapping: {
    ...
    expect(reparsed.scenarios).toEqual(parsed.scenarios);
    expect(stringified).toBe('- [ ] Task #Tasks/Quick #Escenarios/Trabajo #Escenarios/Casa');
  });
  ...
  it('should preserve custom tags when using config', () => {
    ...
    expect(stringify(task, config)).toBe('- [ ] Task #Tareas/Rapida #urgent #backend');
  });
});
...
describe('stringifyArray', () => {
  ...
  it('should support config for all tasks', () => {
    ...
    expect(result).toBe('- [ ] Tarea 1 #Tareas/Mision_Principal\n- [ ] Tarea 2 #Tareas/Rapida');
  });
