 ...
 * ```
 */
export function markAsCompleted(taskString: string, completedDate?: string): string {
  ...
  if (idMatch || depMatch || hookMatch) {
    ...
    const positions = [
      idMatch && idMatch.index !== undefined ? idMatch.index : Number.POSITIVE_INFINITY,
      depMatch && depMatch.index !== undefined ? depMatch.index : Number.POSITIVE_INFINITY,
      hookMatch && hookMatch.index !== undefined ? hookMatch.index : Number.POSITIVE_INFINITY,
    ];
...
export function setEnergy(taskString: string, energy: string): string {
  if (!['high', 'medium', 'low'].includes(energy)) {
    throw new Error(`Invalid energy: ${energy}. Valid values: high, medium, low`);
  }

  let result = taskString.replace(/\s*🌡️\s*(high|medium|low)/g, '');
  result = insertMetadata(result, `🌡️ ${energy}`, ['⏱️', '🔒', '🔺', '⏫', '🔼', '🔽', '⏬']);
  return result.trim();
}
...
export function setDuration(taskString: string, duration: string): string {
  let result = taskString.replace(/\s*⏱️\s*\d+(?:\.\d+)?(?:ms|s|m|h|d|w|y)/gi, '');
...
export function removeDuration(taskString: string): string {
  return taskString.replace(/\s*⏱️\s*\d+(?:\.\d+)?(?:ms|s|m|h|d|w|y)/gi, '').trim();
}
...
export function addFocus(taskString: string, focus: string): string {
  ...
  if (stateMatch && stateMatch.index !== undefined) {
    ...
    const contentStart =
      tagMatch && tagMatch.index !== undefined ? tagMatch.index : taskString.length;
...
export function removeFocus(taskString: string, focus: string): string {
  ...
  return taskString
    .replace(new RegExp(`\\s*${emoji.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}`, 'g'), '')
    .trim();
}
...
export function removeTag(taskString: string, tag: string): string {
  const tagRegex = new RegExp(`\\s*#${tag.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}(?:\\s|$)`, 'g');
 ...
 * ```
 */
export function setTimeRange(taskString: string, start: string, end: string): string {
  if (!/^\d{2}:\d{2}$/.test(start) || !/^\d{2}:\d{2}$/.test(end)) {
    throw new Error(`Invalid time format. Expected HH:MM, got: ${start} - ${end}`);
  }

  let result = taskString.replace(/\s*⏰\s*\[\d{2}:\d{2}\s*-\s*\d{2}:\d{2}\]/g, '');
...
export function removeTimeRange(taskString: string): string {
  return taskString.replace(/\s*⏰\s*\[\d{2}:\d{2}\s*-\s*\d{2}:\d{2}\]/g, '').trim();
}
...
function insertMetadata(taskString: string, metadata: string, beforePatterns: string[]): string {
