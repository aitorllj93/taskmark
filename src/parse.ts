...
import type { Duration, Energy, Focus, Priority, Task, TaskState, TaskType } from './types';
...
export function parse<TScenario extends string = string>(
  ...
  const durationMatch = remaining.match(/⏱️\s*(\d+(?:\.\d+)?(?:ms|s|m|h|d|w|y))/i);
  if (durationMatch?.[1]) {
  ...
  const timeMatch = remaining.match(/⏰\s*\[(\d{2}:\d{2})\s*-\s*(\d{2}:\d{2})\]/);
  ...
  const recurrenceMatch = remaining.match(/🔁\s*([^\n]+?)(?=\s*(?:🆔|⛔|🏁|$))/);
  ...
  const filteredContent = contentChars.filter((char) => !FOCUS_EMOJIS_SET.has(char)).join('');
  ...
  // Validate with Zod
  return TaskSchema.parse(task) as Omit<Task, 'scenarios'> & { scenarios?: TScenario[] };
}
